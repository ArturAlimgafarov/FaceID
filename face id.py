...
def getDB(dbFilename):
    ...
    while sheet.cell(rowIndex, 1).value:
        ...
        db.append(data)
    return db
def getUserFromDB(biometrics, db):
    username = 'unknown'
    if biometrics != None:
        for data in db:
            if len(data) == 129:
                if math.dist(biometrics, data[1:]) < 0.6:
                    username = data[0]
                    break
    return username
...
while True:
    ...
    _, img = capture.read()
    if i >= 20:
        bioData = getBiometrics(img)
        i = 0
    faces = face_cscd.detectMultiScale(img, scaleFactor=1.5, minNeighbors=5, minSize=(50, 50))

    username = getUserFromDB(bioData, db)
    for face in faces:
        (x, y, w, h) = face
        cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 255), 2)
        cv2.putText(img, username, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
